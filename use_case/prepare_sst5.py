...
# ==========================================
SAVE_ROOT = "./sst5_reproducibility"
...
# Using the optimal hyperparameters you provided
MODELS_CONFIG = {
    "bert-base-cased": {"batch_size": 32, "lr": 2e-5, "epochs": 4},
    "bert-large-cased": {"batch_size": 32, "lr": 3e-5, "epochs": 3}
}

# SST-5 specific label mapping
...
def prepare_and_save_dataset():
    print("\n" + "="*40)
    print("PHASE 1: DATASET PREPARATION (SST-5)")
    print("="*40)
    
    print("Loading raw SST-5 dataset from SetFit...")
    ...
    for split_name in dataset.keys():
        ...
        # SST-5 has 'text' and 'label' columns
...
def train_and_save_models(tokenized_dataset):
    ...
    for model_name, config in MODELS_CONFIG.items():
        ...
        print(f"Training {model_name} (Epochs: {config['epochs']}, LR: {config['lr']})...")
        trainer.train()

        print(f"Evaluating {model_name} on Test Set...")
        eval_results = trainer.evaluate(tokenized_dataset["test"])
        print(f"--> Final Test Accuracy for {model_name}: {eval_results['eval_accuracy']:.4f}")
...
if __name__ == "__main__":
    ...
    print("\nAll SST-5 reproducibility tasks completed successfully!")