...
class PASSTrainer(Trainer):
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        climb_window = int(t_total * 0.45)
        reopen_interval = int(t_total * 0.15) # Reopen logic triggered every 20%

        """def _schedule_sparsifier(accumulate_step):
            return coeff_sparsifier_init * 1000 ** (accumulate_step / 1000)"""
            
        def _schedule_sparsifier(accumulate_step):
            # Maintains the exponential growth rate proportional to the run length
            return coeff_sparsifier_init * 1000 ** (accumulate_step / climb_window)
        ...
        def _schedule_clip_reopen_reset(current_step):
            # Proportional trigger: Reopen/Clip after warmup but before training ends
            if warmup_steps < current_step < (t_total * 0.95) and current_step % reopen_interval == 0:
                return True
            return False
