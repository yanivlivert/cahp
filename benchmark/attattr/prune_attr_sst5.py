...
from examples.classifier_processer import InputExample, InputFeatures, DataProcessor, MrpcProcessor, MnliProcessor, RteProcessor, ScitailProcessor, ColaProcessor, SstProcessor, QqpProcessor, QnliProcessor, WnliProcessor, StsProcessor, Sst5Processor
...
def main():
    ...
    #eval_examples = processor.get_dev_examples(args.data_dir, segment=eval_segment)[0:args.num_examples]
    ...
    for baseline_ids, input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Eval (pruned)", total=len(eval_dataloader)):
    ...
    prune_result = []
    temp_time = time.time()
    for heads_kept in [35]:
        total_heads_in_run = 144
